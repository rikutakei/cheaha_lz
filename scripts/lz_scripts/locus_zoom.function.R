...
locus.zoom <- function(data = NULL, snp = NA, gene = NA, region = NA, ld.file = NULL, offset_bp = 200000, genes.data = NULL, psuedogenes = FALSE, RNAs = FALSE, TEC = FALSE, plot.title = NULL, plot.type = "jpg", nominal = 6, significant = 7.3, file.name = NULL, secondary.snp = NA, secondary.label = FALSE, secondary.circle = TRUE, genes.pvalue = NULL, colour.genes = FALSE, population = "EUR", sig.type = "P", nplots = FALSE, ignore.lead = FALSE, rsid.check = TRUE, nonhuman = FALSE, build38 = FALSE, map.data = NULL) {
	...
	# Remove psuedogenes, RNA, and TEC Info, if requested:
	if(!psuedogenes) {
	...
	if (!TEC) {
		genes.data = genes.data[!(genes.data$Coding == "TEC"), ]
	}
...
get.ld <- function(region, snp, population) {
	...
	if (population == 'UKB') {
		...
		if(population == "TAMA"){
			...
			file_path = gsub("POP", population, "/data/project/merrimanlab/reference_files/1kgp_data/per_ancestry/POP/vcf/")
		}
	}
	vcf.filename = gsub(pattern = 'ZZ', replacement = region[1], vcf.filename)
	file_list = list.files(path = file_path)

	# Check if necessary genotype file can be reached
	if(!(vcf.filename %in% file_list)){
		stop(paste0("The file ", paste0(file_path, vcf.filename), " cannot be found."))
	}
	...
	ld.dir = gsub(pattern = 'USER', replacement = Sys.getenv('USER'), ld.dir)

	ld.file = "1KGP_POP_region_ZZ.Y1-Y2_SNP.ld"
	if (population == 'UKB') {
	...
	if (!(ld.file %in% list.files(path = ld.dir, pattern = ".ld"))) {
		messages = system(base.command, ignore.stdout = TRUE, intern = TRUE, ignore.stderr = TRUE)
	}

	# Check the ld file was made & import it
	if (ld.file %in% list.files(path = ld.dir, pattern = ".ld")) {
		ld = read.table(paste(ld.dir, ld.file, sep = ''), stringsAsFactors = FALSE, header = TRUE)
	} else {
...
# variants from the mpa data, but since there is no guarantee that the first
