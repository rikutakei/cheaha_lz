...
sed -e "s/USER/$(whoami)/g" -e "s;NUM;$(wc -l ${PAR_FILE} | awk '{print $1}');g" -e "s;FILE;${PAR_FILE};g" /home/$(whoami)/handy_scripts/locuszooms/scripts/slurm/run_lz.template.sl > /data/scratch/$(whoami)/lz_outputs/slurm_logs/run_lz.$(date +%d%b%y.%H%M%p).sl
